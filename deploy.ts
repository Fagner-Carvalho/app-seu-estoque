...
try {
  ...
  const target = 'mf-auth';

  const directory = 'schedule';

  let { AWS_ACCOUNT_ID, AWS_REGION, ENV_NAME } = process.env;

  if (!ENV_NAME) ENV_NAME = 'local';
  if (!AWS_REGION) AWS_REGION = 'us-east-1';
  if (!AWS_ACCOUNT_ID) AWS_ACCOUNT_ID = 'none';

  const deploy = `s3://${AWS_ACCOUNT_ID}-${AWS_REGION}-${ENV_NAME}-${target}/${directory}`;

  console.log(`started deploy of v${version} to ${deploy}`);

  spawnSync('aws', ['s3', 'sync', './build', deploy], {
